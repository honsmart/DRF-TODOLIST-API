...
class TodosAPITestCase(APITestCase):
       def create_todo(self):
              sample_todo={'title': "GHello bro", 'desc': "The guy don sharp now ooo", }
       ...
       def authenticate(self):
              ...
              response = self.client.post(reverse("login"), {'email': "adegboyeopeyemi580@gmail.com", 
                                                             'password':"honsmart"})
...
class TestListCreateTodos(TodosAPITestCase):
       ...
       def test_should_create_todo(self):
              ...
              self.assertEqual(response.data['title'], 'GHello bro')   
              self.assertEqual(response.data['desc'], 'The guy don sharp now ooo')   


       def test_retrive_all_todos(self):
              self.authenticate()
       ...
       def test_updates_one_item(self):
              ...
              res = self.client.get(
                     reverse("todo", kwargs={'id':response.data['id']}), 
                                    {"title": "GHello bro", 'is_complete':True
                     })
              
              self.assertEqual(res.status_code, status.HTTP_200_OK)

              updated_todo = Todo.objects.get(id=response.data['id'])
              self.assertEqual(updated_todo.is_complete, False)
              self.assertEqual(updated_todo.title, "GHello bro")
